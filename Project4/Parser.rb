...
class Parser < Scanner
    ...
    def exp()
        term_head = term()
        if (@lookahead.type == Token::ADDOP or @lookahead.type::SUBOP)
            tail = etail()
            tail.addChild(term_head)
        else
            tail = term_head
        end
        return tail
    ...
    def term()
        factor_head = factor()
        if (@lookahead.type == Token::MULTOP or @lookahead.type::DIVOP)
            tail = ttail()
            tail.addChild(factor_head)
        else
            tail = factor_head
        end
        return tail
